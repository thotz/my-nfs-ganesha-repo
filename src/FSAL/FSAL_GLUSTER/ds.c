...
{
	int    rc                 = 0;
	struct glfs_ds_handle *ds = container_of(ds_pub, struct glfs_ds_handle, ds);
				...
				struct fsal_ds_handle **const handle)
{
	...
	struct glusterfs_export *glfs_export        =
		container_of(pds->mds_export->fsal_export, struct glusterfs_export, export);
	int p_flags;
	...
	*handle = &ds->ds;
	...
	memcpy (&p_flags, (unsigned char *)hdl_desc->addr + GFAPI_HANDLE_LENGTH, sizeof(p_flags));
